...
class AlternatorDBHandler {
    ...
    protected GetItemResult getItem(GetItemRequest request) throws InternalServerErrorException, ResourceNotFoundException {
        ...
        String keyz = getKeyName(key.getHashKeyElement());
    ...
    protected UpdateItemResult updateItem(UpdateItemRequest request) {
        ...
        if (this.tables.get(tableName).getItem(keyz) == null) {
            ...
            Map<String, AttributeValue> item = this.tables.get(tableName).getItem(keyz);
            ...
            if (attributesToUpdate.size() > 0) {
                for (String k : attributesToUpdate.keySet()) {
                    if (!attributesToUpdate.get(k).getAction().equals("DELETE")) {
                        item.put(k, attributesToUpdate.get(k).getValue());
                        attributesToUpdate.remove(k);
                    }
                }
            }
